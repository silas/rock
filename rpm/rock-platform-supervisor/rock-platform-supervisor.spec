...
mkdir -p %{buildroot}%{_sysconfdir}/rock/platform/supervisord.d

install -p -m 770 -d %{buildroot}%{_localstatedir}/log/%{name}
install -p -D -m 755 %{SOURCE1} %{buildroot}%{_initrddir}/%{name}d
install -p -D -m 644 %{SOURCE2} %{buildroot}%{_sysconfdir}/rock/platform/supervisord.conf
...
%config(noreplace) %{_sysconfdir}/rock/platform/supervisord.conf
%config(noreplace) %{_sysconfdir}/logrotate.d/%{name}
%dir %{_sysconfdir}/rock/platform/supervisord.d
